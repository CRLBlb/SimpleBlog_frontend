      ...
      <el-card class="box-card">
        ...
        <div style="font-size: 20px" class="text item">
          <router-link :to="{path:'/otherhome'
              ,query:{userId: a.comment.userId}}">
            <span style="font-size: 20px"><strong>{{a.userName}}</strong></span>
          </router-link>评论了你的博客《<router-link :to="{path:'/blog/article'
              ,query:{blogId: a.comment.blogId}}">
          <span style="font-size: 20px"><strong>{{a.title}}</strong></span>
        </router-link>》
        </div>
      </el-card>
